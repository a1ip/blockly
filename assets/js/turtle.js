...
/**
 * @fileoverview Черепаха-Blockly, основной файл, описывающий поведение Черепахи
...
function initApplication() { // widthProgram, heightProgram ) {
  ...
  MAXLEVEL = Maps.length - 1; // Level 0 used only for free drawing
...
function initRobot() {
  ...
  turtleImage.src = turtleImageFile;
  turtleImage.onload = function() { display() };
...
function loadProgramFromSessionStorage()  {
  ...
  if( typeof xml_text == 'undefined' ) { 
    ...
    if( typeof xml_text == 'undefined'  ||  xml_text.trim() == '' )
      xml_text = '<xml><block type="turtle_program" deletable="false" movable="false"></block></xml>';
    }
  else {
    if( xml_text.indexOf('turtle_program') < 0 ) {
      xml_text = xml_text.replace( 'xml">' , 'xml">' + 
	        '<block type="turtle_program" deletable="false" movable="false"><next>' );
...
function show() { doTurtleShow( true ); }
function hide() { doTurtleShow( false ); }
function penDown() { doTurtlePen( true ); }
function penUp() { doTurtlePen( false ); }
function forward( steps ) { doTurtleMove( steps ); }
function back( steps ) { doTurtleMove( -steps ); }
function left( degrees ) { doTurtleTurn( -degrees ); }
function right( degrees ) { doTurtleTurn( degrees ); }
function penColour( colour ) { doPenColour( colour ); }
function floodFill( colour ) { doTurtleFill( colour ); }
...
function display() {
  ...
  if ( robotPos['show'] ) {
    ...
    x = xAxisShift + robotPos['x'];
    y = yAxisShift - robotPos['y'];
    ...
    // Draw the turtle head. (4, 1.2, 8, 1)
    radius = 8;
    var WIDTH = 0.4;
    var HEAD_TIP = 8;
    var ARROW_TIP = 2;
    var BEND = 4;
    var radians = Blockly.utils.math.toRadians( robotDirection );
    ...
    radians = Blockly.utils.math.toRadians( robotDirection );
    ctxDisplay.translate( x, y );
    ctxDisplay.rotate( radians ); 
    ctxDisplay.drawImage( turtleImage, -turtleImage.width/2, -turtleImage.height/2 );
    ctxDisplay.rotate( -radians ); 
    ctxDisplay.translate( -x, -y );
    }
...
function grSin( degrees ) {
  var radians = Blockly.utils.math.toRadians( degrees );
  return Math.sin(radians);
}
function grCos( degrees ) {
  var radians = Blockly.utils.math.toRadians( degrees );
  return Math.cos(radians);
}

function doTurtleShow( state ) {
  robotPos['show'] = state;
  display(); 
}

function doTurtlePen( state ) {
  robotPos['pen'] = state;
}
...
function doTurtleFill( colour ) {
  canvasFloodFill( ctxScratch, 
             Math.round( robotPos['x']+xAxisShift ), 
             Math.round( yAxisShift-robotPos['y'] ), colour );
  display();
}

function doTurtleMove( steps ) {

  if( robotPos['pen'] ) {
    ctxScratch.beginPath();
    ctxScratch.moveTo( Math.round(robotPos['x']+xAxisShift) + 0.5, 
                       Math.round(yAxisShift-robotPos['y']) + 0.5 );
    }
  if( steps ) {
    robotPos['x'] += steps * grSin( robotDirection );
    robotPos['y'] += steps * grCos( robotDirection );
  }
  if( robotPos['pen'] ) {
    ctxScratch.strokeStyle = robotColour;
    ctxScratch.lineTo( Math.round(robotPos['x']+xAxisShift) + 0.5, 
                       Math.round(yAxisShift-robotPos['y']) + 0.5 );
    ctxScratch.stroke();
    }
...
function doTurtleTurn( degrees ) {
  robotDirection += degrees;
  display();
}

function scheduleTurtleShow( blockId, state ) {
...
function scheduleTurtleMove( blockId, steps ) {
  addStepPause( blockId );
  programCode.push( [ blockId, 'move', steps ] );
  virtualPos['x'] = virtualPos['x'] + steps * grSin(virtualDirection);
  virtualPos['y'] = virtualPos['y'] + steps * grCos(virtualDirection);
}

function scheduleTurtleTurn( blockId, degrees ) {
  addStepPause( blockId );
  programCode.push( [ blockId, 'turn', degrees ] );
  virtualDirection += degrees;
}

function scheduleTurtleColour( blockId, colour ) {
  addStepPause( blockId );
  programCode.push( [ blockId, 'colour', colour ] );
}

function scheduleTurtleFill( blockId, colour ) {
  addStepPause( blockId );
...
function drawMap( primary = true, variant = 0 ) {
  ...
  startPos = { 'x': 0, 'y': 0, 'show': false, 'pen': false, 'width': 2 };
