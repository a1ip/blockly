...
function getLevel() {
  var thiLevel = parseInt( getParameterByName( 'level', null, '1' ) );
  if( thiLevel < 1  ||  thiLevel > MAXLEVEL) 
  	thiLevel = 1;
  return thiLevel;  
}
...
function initRobot() {
  ...
  workspace.addChangeListener( function() { 
    ...
    if( resetButton.style.display == "none" ) reset();
   	loadCodeToEditor();
	  freshRemainingBlocks();
	} );
...
function sliderChangedCallback( sliderValue ) {
  animationDelay = Math.round(500*Math.exp(-4*sliderValue));
...
{
	Blockly.prompt( "Сохранение файла", "Введите имя файла", '', {}, doSaveFile );
}
...
function runCode( isStepwise = false ) {
  ...
  var remains = workspace.remainingCapacity();
  if( remains < 0 ) {
    Blockly.alert( "Ошибка", 
      "В программе использовано слишком много блоков. " +
...
function animateProgram( isStepwise  = false ) {
	...
	if( programCode.length > 0 ) {
      ...
      if( command == 'moveTo' ) {
        ...
        robotMoveCount += 0.5;
      	...
      	var direction = operation[2];
   	    turnRobot( direction, blockId );
        robotMoveCount += 0.5;
 		}
	...
	if( ! stepPaused && programCode.length > 0 ) 
	  setTimeout(  function() { animateProgram( isStepwise ); }, animationDelay );	
}
...
//======================= USE SESSION STORAGE =======================
function saveProgramToSessionStorage() {
  // MSIE 11 does not support sessionStorage (or localStorage) on file:// URLs.
  if( typeof Blockly == undefined || ! window.sessionStorage ) { return; }  
  ...
  if( errorMessage.length ) {
    ...
    if( LevelSolutionRating == 5 ) 
    	...
    	message = '<table><tr><td><img src="./media/silver-kubok.jpg"></td><td style="padding-left:10px;">' + 
                  message + 
                  '<br>У тебя хорошее решение, оно оценивается в четыре' +
                  '<img src="./media/star.gif" class="star">! Твоя награда&nbsp;&ndash; ' +
                  'серебряный кубок!<br>';
    else
...
function scheduleMove( fromPos, toPos, blockId ) {
  if( ! canMoveTo( toPos.x, toPos.y ) ) {
    ...
    runtimeError = 'Робот не может сделать следующй шаг!';
...
//======================= CHANGE TILE =======================
function changeTileTo( x, y, shape ) {
...
//'   <label for="file-load" style="cursor: pointer;position:relative;top:6px;margin-right:5px;">' +
//'   <img id="upload" src="./media/open.gif" title="Загрузить и открыть сохраненную программу">' +
...
//'        onclick="saveProgramToDisk();" />' +
'   Ты можешь добавить в программу <span id="capacity">0 блоков</span>.' +
'  </div>' +
'</td>' +  
'</tr>' +  
'<tr>' +
